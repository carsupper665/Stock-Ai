...
func (s *SandboxService) SetRuntime(runtime *SandboxRuntimeManager) {
	s.runtime = runtime
}
...
func (s *SandboxService) Get(ctx context.Context, sandboxID string) (*store.Sandbox, error) {
	...
	copy := *sandbox
	copy.ReplayCurrentTime = currentReplayTime(copy)
	return &copy, nil
}
...
func (s *SandboxService) List(ctx context.Context) ([]store.Sandbox, error) {
	...
	for i := range sandboxes {
		sandboxes[i].ReplayCurrentTime = currentReplayTime(sandboxes[i])
	}
...
func (s *SandboxService) Start(ctx context.Context, sandboxID string) (*store.Sandbox, error) {
	...
	if sandbox.Status == store.SandboxStatusStopped {
		return nil, domain.ConflictError("SANDBOX_STOPPED", "stopped sandbox cannot be started")
	}
	runtimeStarted := false
	if s.runtime != nil {
		if err := s.runtime.Start(ctx, sandbox.ID); err != nil {
			return nil, err
		}
		runtimeStarted = true
	}
	current := currentReplayTime(*sandbox)
	now := s.clock.Now()
	...
	if err := s.repo.Save(ctx, sandbox); err != nil {
		if runtimeStarted {
			_ = s.runtime.Stop(context.WithoutCancel(ctx), sandbox.ID)
		}
		return nil, err
	}
	_ = s.events.Publish(ctx, domain.DomainEvent{Topic: "sandbox.updated", SandboxID: sandbox.ID, AggregateID: sandbox.ID, Payload: map[string]any{"status": sandbox.Status}})
...
func (s *SandboxService) Pause(ctx context.Context, sandboxID string) (*store.Sandbox, error) {
	...
	if sandbox.Status == store.SandboxStatusStopped {
		...
		return nil, domain.ConflictError("SANDBOX_STOPPED", "stopped sandbox cannot be paused")
	}
	if s.runtime != nil {
		if err := s.runtime.Pause(ctx, sandbox.ID); err != nil {
			return nil, err
		}
	}
	sandbox, err = s.repo.FindSandbox(ctx, sandboxID)
...
func (s *SandboxService) Stop(ctx context.Context, sandboxID string) (*store.Sandbox, error) {
	...
	if s.runtime != nil {
		if err := s.runtime.Stop(ctx, sandbox.ID); err != nil {
			return nil, err
		}
	}
	sandbox, err = s.repo.FindSandbox(ctx, sandboxID)
...
func (s *SandboxService) Seek(ctx context.Context, sandboxID string, at time.Time) (*ReplayControlStatus, error) {
	...
	if sandbox.Status == store.SandboxStatusRunning {
		return nil, domain.ConflictError("SANDBOX_SEEK_REQUIRES_PAUSE", "pause sandbox before seek")
	}
	current := currentReplayTime(*sandbox)
	materialized := at.UTC()
	if materialized.Before(current) {
		return nil, domain.ValidationError("REPLAY_CURSOR_MONOTONIC", "replay cursor cannot move backward")
	}
...
func (s *SandboxService) HandleRuntimeTick(ctx context.Context, sandboxID string, at time.Time) error {
	if at.IsZero() {
	...
	materialized := at.UTC()
	if materialized.Before(current) {
		return domain.ValidationError("REPLAY_CURSOR_MONOTONIC", "replay cursor cannot move backward")
	}
	...
	_ = s.events.Publish(ctx, domain.DomainEvent{Topic: "sandbox.replay.tick", SandboxID: sandbox.ID, AggregateID: sandbox.ID, Payload: map[string]any{"replay_current_time": materialized}})
	return s.process(ctx, sandbox.ID)
}

func (s *SandboxService) PlanRuntimeTick(ctx context.Context, sandboxID string, elapsed time.Duration) (time.Time, bool, error) {
	if elapsed <= 0 {
		return time.Time{}, false, nil
	}
	sandbox, err := s.repo.FindSandbox(ctx, sandboxID)
	if err != nil {
		return time.Time{}, false, domain.NotFoundError("SANDBOX_NOT_FOUND", "sandbox not found")
	}
	if sandbox.Status != store.SandboxStatusRunning {
		return time.Time{}, false, nil
	}
	...
	advance := time.Duration(float64(elapsed) * speed)
	if advance <= 0 {
		return time.Time{}, false, nil
	}
	return currentReplayTime(*sandbox).Add(advance).UTC(), true, nil
}
