...
export default function PerformanceView({
  ...
  const meanAccountChange = rankedAccounts.length === 0
    ? 0
    : rankedAccounts.reduce((sum, account) => sum + ((account.equity - account.balance) / account.balance) * 100, 0) / rankedAccounts.length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className={`p-4 rounded-xl border ${isDark ? 'bg-[#1b1e25] border-slate-800 text-white' : 'bg-white border-slate-100 text-slate-800'}`}>
          ...
          <span className="text-[10px] text-emerald-500 font-semibold font-mono">
            {peakROI >= 0 ? '+' : ''}{peakROI.toFixed(2)}% backend equity change
          </span>
        </div>
        ...
        <div className={`p-4 rounded-xl border ${isDark ? 'bg-[#1b1e25] border-slate-800 text-white' : 'bg-white border-slate-100 text-slate-800'}`}>
          <div className="flex items-center space-x-1.5 text-sky-500 mb-2">
            <Percent size={14} />
            <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Mean Account Change</span>
          </div>
          <p className="text-xl font-bold font-mono">{meanAccountChange >= 0 ? '+' : ''}{meanAccountChange.toFixed(2)}%</p>
          <span className="text-[10px] text-gray-500">Calculated from backend account equity</span>
        </div>

        <div className={`p-4 rounded-xl border ${isDark ? 'bg-[#1b1e25] border-slate-800 text-white' : 'bg-white border-slate-100 text-slate-800'}`}>
          <div className="flex items-center space-x-1.5 text-amber-500 mb-2">
            <TrendingDown size={14} />
            <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Boundary Reference Zone</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <h4 className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-white' : 'text-slate-800'}`}>
            Sandbox Account Change Comparison (%)
          </h4>
          ...
          <div className={`border p-4 rounded-xl h-64 ${
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={barChartData}>
                ...
                />
                <Bar dataKey="pnlPercent" fill="#2563eb" radius={[4, 4, 0, 0]} name="Account Change %" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
