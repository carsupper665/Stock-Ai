...
import { Dataset, Theme, DemoVisualState } from '../types';
...
export default function DatasetsView({
  ...
  const [formName, setFormName] = useState('SOL-USDT-June-Test');
  const [formSymbol, setFormSymbol] = useState('SOLUSDT');
  const [formInterval, setFormInterval] = useState('1m');
  const [formStart, setFormStart] = useState('2026-06-01');
  const [formEnd, setFormEnd] = useState('2526-06-07');
  const [formSource, setFormSource] = useState('Binance Historical');
  ...
  const handleCreateDataset = (e: React.FormEvent) => {
    ...
    const newDs: Dataset = {
      ...
      name: formName || 'MOCK-DATASET',
      symbol: formSymbol || 'BTCUSDT',
      interval: formInterval || '1m',
      timeRange: `${formStart} to ${formEnd}`,
      source: formSource,
      status: 'Imported',
      updated: 'Just now',
      size: '220 MB'
  ...
  // Mock drag events for file upload UI requirement
  ...
  const handleDrop = (e: React.DragEvent) => {
    e.preventDefault();
    setIsDragging(false);
    // Auto-fill dataset form values on drop!
    setFormName('Dropped_CSV_Replay_Buffer');
    setFormSource('Dropped File Local Metadata');
    setShowForm(true);
  };
  ...
  if (demoState === 'empty' || totalDatasets === 0) {
    return (
      <div className="p-6 text-center max-w-lg mx-auto mt-12">
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className={`p-5 rounded-xl border transition-all ${
          ...
          <p className="text-[10px] text-gray-400 mt-1">Simulated background worker stream</p>
        </div>
        ...
        {showForm && (
          <div className={`w-full lg:w-80 rounded-xl border p-5 space-y-4 shadow-sm h-fit ${
            ...
            <form onSubmit={handleCreateDataset} className="space-y-3">
              <div>
                <label className="block text-[10px] font-semibold text-gray-400 uppercase">Dataset Name</label>
                <input
                  type="text"
                  value={formName}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[10px] font-semibold text-gray-400 uppercase">Symbol Pair</label>
                  <input
                    type="text"
                    value={formSymbol}
              ...
              <div>
                <label className="block text-[10px] font-semibold text-gray-400 uppercase">Data Feed Source</label>
                <input
                  type="text"
                  value={formSource}
              ...
              <button
                type="submit"
                className="w-full py-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded text-xs transition-colors cursor-pointer"
              >
