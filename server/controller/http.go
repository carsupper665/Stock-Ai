...
func (h *Handler) AdminRequired() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		user, err := h.App.Repo.FindUserByID(c.Request.Context(), userID)
		if err != nil || user.Role != store.RoleRootUser {
			writeError(c, domain.ForbiddenError("admin role required"))
			return
		}
		c.Set(ctxAdminID, user.ID)
		c.Next()
	}
}
...
func (h *Handler) SandboxTime(c *gin.Context) {
	accountID := c.MustGet(ctxAccountID).(string)
	account, err := h.App.Accounts.Get(c.Request.Context(), accountID)
	if err != nil {
		writeError(c, err)
	...
	sandbox, err := h.App.Sandboxes.Get(c.Request.Context(), *account.SandboxID)
	if err != nil {
		writeError(c, err)
		return
	}
	c.JSON(http.StatusOK, gin.H{
		...
		"current_time": sandbox.ReplayCurrentTime,
