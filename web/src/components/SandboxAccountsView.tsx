...
import { SandboxAccount, Theme, ViewState } from '../types';
...
export default function SandboxAccountsView({
  ...
  const [accBalance, setAccBalance] = useState('10000.00');
  const [targetSandbox, setTargetSandbox] = useState(sandboxes[0]?.id || '');

  useEffect(() => {
    if (targetSandbox === '' && sandboxes[0]?.id) {
      setTargetSandbox(sandboxes[0].id);
    }
  }, [sandboxes, targetSandbox]);
  ...
  const handleProvision = async (e: React.FormEvent) => {
    ...
    setActionError(null);
    try {
      const account = await createSandboxAccount(targetSandbox, {
        name: accName,
        initialBalance: parseFloat(accBalance) || 0,
      });
      const tokenSecret = await createToken({
        accountId: account.id,
        name: `${accName} Token`,
        scopes: ['market:read', 'trade:write'],
      });
      const accountWithToken: SandboxAccount = {
        ...account,
        token: tokenSecret.token,
        tokenId: tokenSecret.id,
        tokenRevealed: false,
      };
      onAddAccount(accountWithToken);
      setShowForm(false);
      setActiveTokenReveal(accountWithToken);
    } catch (error) {
      setActionError(error instanceof Error ? error.message : 'Unable to provision sandbox account.');
    } finally {
  ...
  const handleRevokeToken = async (id: string) => {
    ...
    if (acc) {
      ...
      setActionError(null);
      try {
        await revokeToken(acc.tokenId || acc.token || id);
        const updated = {
          ...acc,
          token: undefined,
          tokenRevoked: true,
        ...
        setActionError(error instanceof Error ? error.message : 'Unable to revoke token.');
      } finally {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col xl:flex-row gap-6">
        ...
        {/* Dynamic Warning Card / One-Time Token Reveal Panel */}
            ...
            {showForm && (
              <div className="p-5 rounded-xl border shadow-sm bg-app-card border-app-border text-app-text-main">
                ...
                <form onSubmit={handleProvision} className="space-y-3">
                  ...
                  <div>
                    <label className="block text-[10px] font-semibold text-gray-400 uppercase">Initial Balance (USD)</label>
                    <input
                      type="number"
                      value={accBalance}
                  ...
                  <button
                    ...
                    className="w-full py-2 bg-app-accent text-white text-xs font-semibold rounded cursor-pointer hover:bg-app-accent/90 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    {isProvisioning ? 'Generating Credentials...' : 'Generate Backend Credentials'}
                  </button>
                </form>
              </div>
            )}
