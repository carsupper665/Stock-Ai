...
import { Sandbox, SandboxAccount, OperationalLog, Theme, DemoVisualState } from '../types';
...
export default function SandboxMonitor({
  ...
  const [selectedSbId, setSelectedSbId] = useState<string>(sandboxes[0]?.id || 'sb-01');
  ...
  // Setup sample Recharts trend data
  const defaultPerformanceHistory = [
    { time: '10:00', value: 10000 },
    { time: '10:15', value: 10050 },
    { time: '10:30', value: 9980 },
    { time: '10:45', value: 10140 },
    { time: '11:00', value: 10245 },
    { time: '11:15', value: 10190 },
    { time: '11:30', value: 10320 },
    { time: '11:45', value: 10450 }
  ];

  const trendData = activeSandbox?.equityTrend || defaultPerformanceHistory;
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {relatedAccounts.length === 0 ? (
                <div className={`p-4 text-center rounded-xl border text-xs text-gray-400 ${
                  ...
                  No simulated accounts attached to this sandbox.
                </div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-right font-mono">
                        ...
                        <p className={`text-[10px] font-semibold ${isProfit ? 'text-emerald-500' : 'text-red-500'}`}>
          ...
          <div className="space-y-3">
            ...
            <div className="p-3 bg-amber-500/5 border border-amber-500/10 text-amber-600 dark:text-amber-500 rounded-lg text-[10px] leading-relaxed mb-3">
              Indicators are derived from replay historical cache frames for simulated trigger reconciliation. They do not constitute actual trading recommendations or strategy builders.
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              
              <div className={`p-3 rounded-lg border ${
                demoState === 'empty' ? 'opacity-40' : ''
              } ${isDark ? 'bg-slate-900/40 border-slate-800 text-white' : 'bg-slate-50 border-slate-200 text-slate-800'}`}>
                <p className="text-[9px] uppercase font-bold text-slate-400">Sim Moving Avg (14)</p>
                <p className="text-sm font-bold font-mono mt-1 text-sky-500">
                  {demoState === 'empty' ? 'NO DATA' : '$67,420.50'}
                </p>
                <span className="text-[9px] text-gray-400">Dynamic SMA calculation</span>
              </div>

              <div className={`p-3 rounded-lg border ${
                demoState === 'empty' ? 'opacity-40' : ''
              } ${isDark ? 'bg-slate-900/40 border-slate-800 text-white' : 'bg-slate-50 border-slate-200 text-slate-800'}`}>
                <p className="text-[9px] uppercase font-bold text-slate-400">RSI Indicator</p>
                <p className="text-sm font-bold font-mono mt-1 text-emerald-500">
                  {demoState === 'empty' ? 'NO DATA' : '54.81'}
                </p>
                <span className="text-[9px] text-gray-400">Neutral threshold zone</span>
              </div>

              <div className={`p-3 rounded-lg border ${
                demoState === 'empty' ? 'opacity-40' : ''
              } ${isDark ? 'bg-slate-900/40 border-slate-800 text-white' : 'bg-slate-50 border-slate-200 text-slate-800'}`}>
                <p className="text-[9px] uppercase font-bold text-slate-400 font-mono">24h Sim Volatility</p>
                <p className="text-sm font-bold font-mono mt-1 text-slate-500">
                  {demoState === 'empty' ? 'NO DATA' : '4.21%'}
                </p>
                <span className="text-[9px] text-gray-400">Stablized mean dispersion</span>
              </div>
